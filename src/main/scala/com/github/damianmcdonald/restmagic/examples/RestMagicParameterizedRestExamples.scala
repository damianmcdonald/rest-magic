...
class RestMagicParameterizedRestExamples extends RegistrableMock with ConfiguratorUtils {
  ...
  private val mastersOfTheUniverseByParamApi = ParameterizedRestConfig(
    httpMethod = GET,
    apiPath = rootApiPath / "examples" / "parameterizedrest" / "motu" / "html" / "file" / MATCH_PARAM,
    produces = `text/html`,
    dataMode = FileStub(),
    responseData = Map(
      "1" -> "/examples/parameterizedrest/he-man.html",
      "2" -> "/examples/parameterizedrest/skeletor.html",
      "3" -> "/examples/parameterizedrest/man-at-arms.html",
      "4" -> "/examples/parameterizedrest/beast-man.html",
      "5" -> "/examples/parameterizedrest/orko.html"
    ),
    serveMode = ByParam(),
    validate = true,
    displayName = "Parameterized Rest Masters of the Universe Html File",
    displayUrl = "/" + rootApiPath + "/examples/parameterizedrest/motu/html/file/{{characterId}}"
  )
  ...
  private val heManSingularApi = ParameterizedRestConfig(
    ...
    produces = `text/html`,
    dataMode = FileStub(),
    responseData = Map("1" -> "/examples/parameterizedrest/he-man.html"),
    serveMode = Singular(),
    validate = true,
